...
template <typename T>
set<T> operator+(const set<T>& lhs, const set<T>& rhs){                     // <합집합> 
    set<T> ret;
    for(typename set<T>::iterator it= lhs.begin();it!=lhs.end();it++)       // for문 돌릴때 typename을 써줘야 error가 나지않는다. 
        ret.insert(*it);                                                    // 더하기 일때는 ret이라는 set을 만들어 거기다가 lhs, rhs모두추가한다 
...
template <typename T>
set<T> operator-(const set<T>& lhs, const set<T>& rhs){                     // < 차집합 > 
    set<T> ret;
    for(typename set<T>::iterator it= lhs.begin();it!=lhs.end();it++)       // lhs set을 ret에 다넣어준후  
        ret.insert(*it);
...
template <typename T>
set<T> operator*(const set<T>& lhs, const set<T>& rhs){                       // < 교집합 > 
    set<T> ret;
...
template <typename T>
ostream& operator<<(ostream& os, const set<T>& s){                            //set 출력시 괄호하고 그 안에 원소들을 차례로 출력시켜준다 
...
template <typename T>
istream& operator>>(istream& is, set<T>& s){                                  //set 입력시 먼저 크기를 입력받고 그크기만큼 set에다가 저장해준다 
    T size, number;                                                            
    cin>>size;
